...
public class KafkaOffsetGen {
  ...
  public OffsetRange[] getNextOffsetRanges(Option<String> lastCheckpointStr, long sourceLimit) {
    ...
    //}
    Set<TopicPartition> topicPartitions = partitionInfoList.stream().map(
        x -> new TopicPartition(x.topic(), x.partition())).collect(Collectors.toSet());
    ...
    OffsetRange[] offsetRanges = CheckpointUtils.computeOffsetRanges((HashMap<TopicPartition, Long>) fromOffsets,
                                                                     (HashMap<TopicPartition, Long>) toOffsets, numEvents);
